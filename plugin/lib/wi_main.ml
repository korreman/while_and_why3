...
let file_parser (_env : Env.env) (_path : Env.pathname) (_file : Env.filename)
    (c : in_channel) : Wi_ast.ast =
...
let () =
  Env.register_format ~desc:"While format" while_language "while" [ "wi" ]
    file_parser
...
let () =
  Args_wrapper.set_argument_parsing_functions "while"
    ~parse_term:Wi_parser.parse_term ~parse_term_list:Wi_parser.parse_term_list
